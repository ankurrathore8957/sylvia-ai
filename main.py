...
class CyberAssistantUI:
    ...
    def __init__(self) -> None:
        self.root = tk.Tk()
        self.root.title("SYLVIA Voice Assistant")
        ...
        self.accent_primary = "#65e7ff"
        self.bg_main = "#030611"
        self.panel_fill = "#08111f"
        self.panel_line = "#183456"
    ...
    def _build_chat_widgets(self) -> None:
        ...
        self.chat_output = tk.Text(
            ...
            bg="#050c17",
            fg="#eaf3ff",
            insertbackground="#eaf3ff",
            bd=0,
            highlightthickness=1,
            highlightbackground="#17314f",
            highlightcolor="#65e7ff",
            font=("Helvetica", 10),
        )
        self.chat_output.configure(state="disabled")
        self.chat_output.tag_configure("user_label", foreground="#76e7ff", font=("Helvetica", 10, "bold"))
        self.chat_output.tag_configure("assistant_label", foreground="#7f8bff", font=("Helvetica", 10, "bold"))
        self.chat_output.tag_configure("user_text", foreground="#eaf3ff")
        self.chat_output.tag_configure("assistant_text", foreground="#dbe8f8")
        self.chat_output.tag_configure("placeholder", foreground="#8fb4d8")
        ...
        self.send_button = tk.Button(
            ...
            bg="#0d1930",
            fg="#eaf3ff",
            activebackground="#13294a",
            activeforeground="#eaf3ff",
            bd=0,
            padx=14,
            pady=8,
        ...
        self.copy_button = tk.Button(
            ...
            bg="#07111f",
            fg="#9cd8ff",
            activebackground="#0d1930",
            activeforeground="#9cd8ff",
            bd=0,
            padx=14,
            pady=8,
            font=("Helvetica", 9, "bold"),
    ...
    def draw_background(self, t: float) -> None:
        ...
        glow_specs = [
            (self.width * 0.24, self.height * 0.18, 340, 180, "#0b2340"),
            (self.width * 0.76, self.height * 0.22, 300, 150, "#101c4c"),
            (self.width * 0.50, self.height * 0.48, 520, 180, "#081a2f"),
        ]
        ...
        self.canvas.create_rectangle(0, horizon, self.width, self.height, fill="#040813", outline="")

        vanish_x = self.width / 2
        for i in range(-12, 13):
            px = vanish_x + i * self.width * 0.06
            self.canvas.create_line(vanish_x, horizon, px, self.height, fill="#0d2741", width=1)
        ...
        for i in range(11):
            ...
            color = "#12314d" if i % 2 == 0 else "#0d2440"
        ...
        self.canvas.create_line(arc_points, fill="#133250", smooth=True, width=2)
        ...
        self.canvas.create_text(
            ...
            self.height - 26,
            anchor="w",
            text="SYLVIA VOICE INTERFACE // LOW LATENCY VISUAL ENGINE",
            fill="#46688b",
            font=("Helvetica", 10, "bold"),
        )
    ...
    def draw_layout_panels(self, t: float) -> None:
        ...
        lx = margin + 22
        self.canvas.create_text(lx, top_y + 46, anchor="w", text=self.time_text, fill="#f2f8ff", font=("Helvetica", 30, "bold"))
        self.canvas.create_text(lx, top_y + 80, anchor="w", text=self.date_text, fill="#8fb4d8", font=("Helvetica", 12))
        self.canvas.create_text(lx, top_y + 104, anchor="w", text=self.clip_text(f"{self.fps_text}   •   {self.device_text}", 40), fill="#6e88aa", font=("Helvetica", 10, "bold"))
        self.canvas.create_text(lx, top_y + 123, anchor="w", text=self.clip_text(self.stats_text, 42), fill="#6f89a8", font=("Helvetica", 10))

        rx = self.width - right_w - margin + 22
        self.canvas.create_text(rx, top_y + 46, anchor="w", text=self.clip_text(self.battery_text, 28), fill="#f2f8ff", font=("Helvetica", 16, "bold"))
        self.canvas.create_text(rx, top_y + 74, anchor="w", text=self.clip_text(self.weather_text, 38), fill="#9cd8ff", font=("Helvetica", 11))
        self.canvas.create_text(rx, top_y + 103, anchor="w", text=self.clip_text(self.network_text, 38), fill="#6f89a8", font=("Helvetica", 10))
        self.canvas.create_text(rx, top_y + 122, anchor="w", text="MIC STATE: " + ("ACTIVE" if self.listening else "STANDBY"), fill="#88b0d8", font=("Helvetica", 10, "bold"))

        lower_x = margin + 22
        self.canvas.create_text(lower_x, self.height - lower_h, anchor="w", text="VOICE ENGINE", fill="#dbe8f8", font=("Helvetica", 15, "bold"))
        self.canvas.create_text(
            lower_x,
    ...
    def draw_glass_panel(self, x1: float, y1: float, x2: float, y2: float, title: str) -> None:
        ...
        self.canvas.create_rectangle(x1 + 1, y1 + 1, x2 - 1, y1 + 18, fill="#0d1930", outline="")
        self.canvas.create_line(x1 + 14, y1 + 18, x2 - 14, y1 + 18, fill="#122948", width=1)

        cut = 14
        self.canvas.create_line(x1, y1 + cut, x1, y1, x1 + cut, y1, fill="#5f87bc", width=2)
        self.canvas.create_line(x2 - cut, y1, x2, y1, x2, y1 + cut, fill="#5f87bc", width=2)
        self.canvas.create_line(x1, y2 - cut, x1, y2, x1 + cut, y2, fill="#244364", width=2)
        self.canvas.create_line(x2 - cut, y2, x2, y2, x2, y2 - cut, fill="#244364", width=2)

        self.canvas.create_text(x1 + 16, y1 + 9, anchor="w", text=title, fill="#9bb7db", font=("Helvetica", 9, "bold"))

    def draw_level_meter(self, x: float, y: float, width: float, height: float, t: float, active: bool) -> None:
        self.canvas.create_rectangle(x, y, x + width, y + height, fill="#07111f", outline="#17314f", width=1)
        segments = 16
        gap = 3
        ...
        for i in range(segments):
            ...
            fill = self.mix_color("#16314e", "#76e7ff", intensity)
    ...
    def draw_analog_clock(self) -> None:
        ...
        self.canvas.create_oval(cx - r - 14, cy - r - 14, cx + r + 14, cy + r + 14, fill="#050d18", outline="")
        self.canvas.create_oval(cx - r, cy - r, cx + r, cy + r, fill="#081321", outline="#21476f", width=2)
        self.canvas.create_oval(cx - r * 0.84, cy - r * 0.84, cx + r * 0.84, cy + r * 0.84, outline="#122948", width=1)

        for i in range(60):
            angle = math.radians(i * 6 - 90)
            inner = r * (0.76 if i % 5 == 0 else 0.86)
            outer = r * 0.94
            color = "#d8e9ff" if i % 5 == 0 else "#46698d"
        ...
        self.draw_hand(cx, cy, r * 0.46, hour * 30 - 90, "#ffffff", 4)
        self.draw_hand(cx, cy, r * 0.66, minute * 6 - 90, "#8ee6ff", 3)
        self.draw_hand(cx, cy, r * 0.80, second * 6 - 90, "#7f8bff", 2)

        self.canvas.create_oval(cx - 5, cy - 5, cx + 5, cy + 5, fill="#f4fbff", outline="")
        self.canvas.create_text(cx, cy + r + 18, text="CLOCK", fill="#85a8d2", font=("Helvetica", 10, "bold"))
    ...
    def draw_assistant_core(self, t: float) -> None:
        ...
        ring_colors = ["#10233e", "#173455", "#204265"]
        ...
        if self.listening:
            for i in range(3):
                ...
                line = self.mix_color("#244364", self.accent_primary, 0.35 + i * 0.18)
                self.canvas.create_oval(cx - rr, cy - rr, cx + rr, cy + rr, outline=line, width=2)

        disk_r = base_r * pulse
        self.canvas.create_oval(cx - disk_r, cy - disk_r, cx + disk_r, cy + disk_r, fill="#050c17", outline="#2d4e73", width=2)
        ...
        self.canvas.create_oval(
            ...
            cy + inner_r,
            fill="#081525",
            outline=self.mix_color("#27415f", "#96f0ff", 0.9 if self.listening else 0.22),
            width=2,
        )
        ...
        self.canvas.create_oval(
            ...
            cx + core_r,
            cy + core_r,
            fill=self.mix_color("#15344e", "#76e7ff", 0.88 if self.listening else 0.28),
            outline="",
        )
        ...
        for i in range(48):
            ...
            color = self.mix_color("#264568", "#7ce8ff", 0.85 if self.listening else 0.18)
            self.canvas.create_line(x1, y1, x2, y2, fill=color, width=2 if i % 6 == 0 else 1)

        self.canvas.create_text(cx, cy - base_r - 58, text="VOICE CORE", fill="#8ba6c9", font=("Helvetica", 11, "bold"))
        self.canvas.create_text(cx, cy + base_r + 34, text="tap to toggle microphone", fill="#85a0c3", font=("Helvetica", 11))
        self.canvas.create_text(
            cx,
            cy + base_r + 60,
            text="LISTENING" if self.listening else "IDLE",
            fill="#aef8ff" if self.listening else "#91a4bd",
        ...
        for i, color in enumerate(self.orb_palette):
            ...
            self.canvas.create_oval(ox - size * 2.2, oy - size * 2.2, ox + size * 2.2, oy + size * 2.2, outline=self.mix_color("#223a59", color, 0.55), width=1)
            self.canvas.create_oval(ox - size, oy - size, ox + size, oy + size, fill=self.mix_color("#385070", color, 0.78), outline="")

            if self.listening:
                tail_x = cx + math.cos(angle - 0.06) * (local_r - 10)
                tail_y = cy + math.sin(angle - 0.06) * ((local_r - 10) * 0.72)
                self.canvas.create_line(tail_x, tail_y, ox, oy, fill=self.mix_color("#2a4060", color, 0.45), width=1)
    ...
    def draw_popup(self, t: float) -> None:
        ...
        self.canvas.create_rectangle(x1, y1, x2, y2, fill="#091321", outline="#2a4d73", width=1)
        self.canvas.create_rectangle(x1 + 1, y1 + 1, x2 - 1, y1 + 16, fill="#102038", outline="")
        self.canvas.create_line(x1 + 14, y1 + 16, x2 - 14, y1 + 16, fill="#17314f", width=1)

        self.canvas.create_text(cx, y1 + 31, text=self.popup_title, fill="#f1f7ff", font=("Helvetica", 13, "bold"))
        self.canvas.create_text(cx, y1 + 57, text=self.popup_text, fill="#8ee6ff", font=("Helvetica", 18, "bold"))
